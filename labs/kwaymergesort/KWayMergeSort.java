		...
		while( true ){
			...
			for ( int i = 0; i < mergedSubArrays.length; ++i ){
				...
				for ( int j = 0; j < mergedSubArrays[0].length; ++j ){
					...
					// true when elements in the 1st subarray are used up (pick those in 2nd)
					if( k > size-1 ){
						mergedSubArrays[i][j] = subArrays[i*2+1][l];
						l++;
					}
					// true when elements in the 2nd subarray are used up (pick those in 1st)
					else if( l > size-1 ){
			...
			numArray /= 2;
			size *= 2;
			// merge complete if there's only one subarray
			if( numArray == 1 ){
